...
module.exports.set_const = function(gConst) {
  ...
  // カードの場のx座標, カードの場の仕切り線のx座標
  for (let i=1; i<=10; i++) {
    gConst.card.base_x[i] = gConst.layout.base_x+gConst.layout.card_x*(2*i-1)/2+gConst.layout.interval*(i-1);
    gConst.card.line_x[i] = gConst.card.base_x[i]+0.5*(+gConst.layout.card_x+gConst.layout.interval);
  }
  gConst.card.base_x[0] = 900;
