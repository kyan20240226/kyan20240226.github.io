...
module.exports.set_const = function(gConst) {
  ...
  // ブロック描写用のscale
  gConst.block.scale = gConst.size.block/Math.sqrt(2*(gConst.block.width**2));
  gConst.num.scale = 0.4*gConst.size.block/gConst.num.width;
  ...
  // ブロックの最大個数
  gConst.block.num = gConst.floor.max_col*gConst.floor.max_raw;
...
// onPointUpでポインタが対象オブジェクト上に残っているかどうかの判定
module.exports.jud_ontarget = function(target, ev, op) {
  let x=ev.point.x+ev.startDelta.x;
  let y=ev.point.y+ev.startDelta.y;
  let jud_x=1;
  let jud_y=1;
  if (x < 0 || x > target.width) { jud_x = 0 }
  if (y < 0 || y > target.height) { jud_y = 0 }
  let jud=jud_x+jud_y;
  return jud;
};
